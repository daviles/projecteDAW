...
class controlador extends ModelPDO {
    ...
    function __construct() {
        ...
        if (isset($_SESSION['usuario'])) {  // si hay un usuario en sesion...
            ...
            if (isset($_POST['modificaInventario'])) {
                $this->instartaInventario($_POST['id'], $_POST['descripcion'], $_POST['unidad'], $_POST['precio'], $_POST['coleccion'], $_POST['zona']);
            }
    ...
    function generaHTML($zonas, $items) {
        ...
        for ($i = 0; $i < $zonas; $i++) {
            ...
            $html .= '<th>Nombre Zona: ' . $_SESSION['zonas'][$i][0] . '</th>';
            $html .= '<td>Unidad</td>';
            $html .= '<td>Precio</td>';
            $html .= '<td>Cantidad</td>';
            $html .= '<td>Total</td>';
            ...
            for ($f = 1; $f < $items; $f++) {
                ...
                $html .= '<td onchange="suma('.$item[0]['idInventario'].','.$i.')"><input type="text" name="cantidad"'
                        . 'id="item'.$item[0]['idInventario'].'"></td>';
                $html .= '<td id="totalItem'.$item[0]['idInventario'].'"></td>';
                $html .= '</tr>';
                
            }
            $html .= '<tr><th>Total Zona</th><td colspan="4" id="totalZona'.$i.'"></td></tr>';
            //separador
